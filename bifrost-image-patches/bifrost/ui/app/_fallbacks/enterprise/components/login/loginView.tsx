...
export default function LoginView() {
	...
	const isSSOEnabled = isAuthEnabledData?.sso_enabled || false;
	const ssoLoginUrl = isAuthEnabledData?.sso_login_url || "/api/session/sso/login";
					...
					<form onSubmit={handleSubmit} className="space-y-5">
						...
						{isSSOEnabled && (
							<div className="space-y-4">
								<Button type="button" className="h-9 w-full gap-2 text-sm" onClick={handleSSOLogin}>
									<KeyRound className="h-4 w-4" />
									Sign in with Keycloak
								</Button>
								<div className="flex items-center gap-3">
									<div className="bg-border h-px flex-1" />
									<span className="text-muted-foreground text-xs">or use local admin</span>
									<div className="bg-border h-px flex-1" />
								</div>
							</div>
						)}
