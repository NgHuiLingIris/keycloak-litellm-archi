...
export default function AppSidebar() {
  ...
  const sessionUser = sessionProfileData?.user;
  const displayUser =
    sessionUser && (sessionUser.name || sessionUser.email || sessionUser.avatar_url)
      ? {
          name: sessionUser.name || sessionUser.email || "User",
          email: sessionUser.email,
          avatarUrl: sessionUser.avatar_url,
        }
      : userInfo && (userInfo.name || userInfo.email || userInfo.picture)
        ? {
        ...
        <div className="flex flex-col gap-4 px-3 group-data-[collapsible=icon]:px-1">
          ...
          <div className="flex flex-row">
            <div className="mx-auto flex flex-row gap-4 group-data-[collapsible=icon]:flex-col group-data-[collapsible=icon]:gap-2">
              ...
              <ThemeToggle />
              {isAuthEnabled && displayUser ? (
                <Popover open={userPopoverOpen} onOpenChange={setUserPopoverOpen}>
                  <PopoverTrigger asChild>
                    <button
                      className="hover:text-primary text-muted-foreground flex cursor-pointer items-center space-x-3 rounded-full p-0.5"
                      type="button"
                      aria-label="User menu"
                    >
                      <Avatar className="h-7 w-7 border">
                        {displayUser.avatarUrl && <AvatarImage src={displayUser.avatarUrl} alt={displayName} />}
                        <AvatarFallback className="text-[11px] font-medium">{getInitials(displayName)}</AvatarFallback>
                      </Avatar>
                    </button>
                  </PopoverTrigger>
                    ...
                    <div className="flex flex-col">
                      <div className="flex items-center gap-3 px-4 py-3">
                        ...
                        <div className="min-w-0">
                          ...
                          {displayUser.email && (
                            <p className="text-muted-foreground truncate text-xs">{displayUser.email}</p>
                          )}
                        </div>
                      </div>
                      <Separator />
