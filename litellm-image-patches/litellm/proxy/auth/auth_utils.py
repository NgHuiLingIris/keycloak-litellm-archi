...
def get_free_sso_user_limit() -> int:
    raw_limit = os.getenv("LITELLM_FREE_SSO_USER_LIMIT", "5").strip()
    try:
        limit = int(raw_limit)
    except ValueError:
        verbose_proxy_logger.warning(
            "Invalid LITELLM_FREE_SSO_USER_LIMIT=%r. Falling back to 5.",
            raw_limit,
        )
        return 5
    if limit < 0:
        verbose_proxy_logger.warning(
            "LITELLM_FREE_SSO_USER_LIMIT must be >= 0. Falling back to 5."
        )
        return 5
    return limit
