...
from litellm.proxy.auth.auth_utils import _has_user_setup_sso, get_free_sso_user_limit
    ...
    if (
        ...
        if premium_user is not True:
            ...
            if prisma_client is not None:
                ...
                free_sso_user_limit = get_free_sso_user_limit()
                if (
                    free_sso_user_limit > 0
                    and total_users
                    and total_users > free_sso_user_limit
                ):
                    raise ProxyException(
                        message=f"You must be a LiteLLM Enterprise user to use SSO for more than {free_sso_user_limit} users. If you have a license please set `LITELLM_LICENSE` in your env. If you want to obtain a license meet with us here: https://calendly.com/d/cx9p-5yf-2nm/litellm-introductions You are seeing this error message because You set one of `MICROSOFT_CLIENT_ID`, `GOOGLE_CLIENT_ID`, or `GENERIC_CLIENT_ID` in your env. Please unset this",
